...
class Topcmd(Cmd):
    ...
    def subcmd_id_merge(self, subcmd, id, res, resc):
        gstats = res[subcmd + 's']
    ...
    def subcmd_show(self, line, key='bridge', stats='stats', id=None):
        args = line.split()
        try:
            subcmd = args[0]
        except:
        ...
        if len(args) >= 2:
            ...
            if id != None:
                ...
                self.subcmd_id_merge(subcmd, id, res, resc)
            if len(args) == 3:
                try:
                    res = res[args[2]]
                except:
            ...
            try:
                ...
                for ifdata in data:
                    ...
                    res = ds_client().call(req)[0]
                    if id != None:
                        resc = ds_client().call(subcmd + ' ' + name.encode() + '\n')[0]
                        self.subcmd_id_merge(subcmd, id, res, resc)
                    try:
                        res['name'] = name
...
def usage():
    print 'Usage: ' + sys.argv[0] + '[--dsl-decode [file]] [--dsl-encode [file]]'
